...
def plot_and_save(axis_1, axis_2, iter, NTH, STH):
	...
	plt.savefig("data/main_random/24/IB_BIN_ITERATION%s.png" % iter)
	plt.close()
	...
	plt.savefig("data/main_random/24/TRAIN_ITERATION%s.png" % iter)
	plt.close()
		...
		if num_crt >= NUM_CRT:
			...
			if INFORMATION:
				...
				if NUM_CRT >= 3500 and check:
					np.save('data/main_random/24/DATA_ITERATION%s.npy' % iteration, DATA)
					np.save('data/main_random/24/W_ITERATION%s.npy' % iteration, W_HISTORY)