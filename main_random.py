...
def plot_and_save(axis_1, axis_2, iter, NTH, STH):
	...
	plt.savefig("data/main_random/12/IB_BIN_ITERATION%s.png" % iter)
	plt.close()
	...
	plt.savefig("data/main_random/12/TRAIN_ITERATION%s.png" % iter)
	plt.close()
	...
	while True:
		...
		if num_crt >= NUM_CRT:
			...
			if INFORMATION:
				...
				W_HISTORY.append([W_1,0, W_2])  # there's a broadcast bug for 12 case, so I add a zero

				if PLOT:
					plot_and_save(DATA["axis_1"], DATA["axis_2"],
								  iteration, DATA["NUM_CRT_HISTORY"], DATA["STEPS_HISTORY"])
				if NUM_CRT >= 3500 and check:
					np.save('data/main_random/12/DATA_ITERATION%s.npy' % iteration, DATA)
					np.save('data/main_random/12/W_ITERATION%s.npy' % iteration, W_HISTORY)
